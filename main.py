...
def finish_reg():
    ...
    for name_check in all_accounts:
        if name==name_check:
            notif.config(fg="red",text="Accounts already exists")
            return
        else:
...
def login_session():
    global login_name,login_password
    all_accounts = os.listdir()
    ...
    for name in all_accounts:
       if name == login_name:
       # if not os.path.isdir(name) and name!="main.py":
            file=open(name,"r")
            file_data = file.read()
...
def deposit():
    global amount , current_balance_Label , deposit_notif
    amount=StringVar()
...
def finish_deposit():
    ...
    file_data = file_data.replace(current_balance,str(round(updated_balance,2)))
    file.seek(0)
    file.truncate(0)
...
def withdraw():
    global withdraw_amount, withdraw_current_balance_Label, withdraw_notif
    ...
    withdraw_current_balance_Label = Label(withdraw_screen, text='Current Balance: $' + details_balance, font=('Calibri', 12))
    withdraw_current_balance_Label.grid(row=1, sticky=W)
...
def finish_withdraw():
    ...
    current_balance= details[4]
    updated_balance = current_balance
    if float(withdraw_amount.get()) > float(updated_balance):
    ...
    withdraw_current_balance_Label.config(text="Current Balance: $"+str(round(updated_balance,2)),fg='green')
...
#Image import...
img= Image.open('./photo/banking.png')
img = img.resize((150,150))
img = ImageTk.PhotoImage(img)

#Labels
Label(master, text="Banking System",font=('Calibri',12)).grid(row=0,sticky=N)
Label(master, text="The Most secured Bank you've probably used",font=('Calibri',12)).grid(row=1,sticky=N,pady=10)
Label(master,image=img).grid(row=2,sticky=N,pady=15)

#Buttons
Button(master, text="Register",font=('Calibri',12),width=10,command=register).grid(row=3,sticky=N)
Button(master, text="Login",font=('Calibri',12),width=10,command=login).grid(row=4,sticky=N)
